...
const MAP_SYMBOLS = new Map([
  ...
  ['$', ((options = {}) => ({ ...{ symbol: '$', tileTypeId: 3, tileType: 'start', direction: null, isCharacter: false, isPassable: true, isExit: true }, ...options })).bind(this)],
  ['x', ((options = {}) => ({ ...{ symbol: 'x', tileTypeId: 4, tileType: 'exit', direction: null, isCharacter: false, isPassable: true, isExit: true }, ...options })).bind(this)],
  ['.', ((options = {}) => ({ ...{ symbol: '.', tileTypeId: 5, tileType: 'path', direction: null, isCharacter: false, isPassable: true, isExit: false }, ...options })).bind(this)],
  ['@', ((options = {}) => ({ ...{ symbol: '@', tileTypeId: 6, tileType: 'target', direction: null, isCharacter: false, isPassable: true, isExit: true }, ...options })).bind(this)],
]);
...
export class MazeModel {
  ...
  print(showPath = false) {
    ...
    const output = this.#map.reduce((string, row, i) => {
      ...
      if (tile.isTarget == true && !tile.hasCharacter) { return '@'; } 
        
        return tile.hasCharacter === true ? 
        `
  ...
  set targetTile(tile) {
    if (this.#targetTile) {
      this.targetTile.isTarget = false
    }

    this.#targetTile = tile
    this.#targetTile.isTarget = true
  ...
  setTargetTile(row, column) {
    ...
    let pathNode = this.shortestPathDfs(this.characterTile, this.targetTile);
